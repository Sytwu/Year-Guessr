...
class RegressionDataModule(pl.LightningDataModule):
    def __init__(
        ...
        self.test_set = RegressionDataset(test_images_root, test_data_file, eval_transforms, test_location)
...
class RegressionDataset(Dataset):
    def __init__(self, images_root, data_file, transforms=None, test_location=True):
        ...
        with open(data_file) as fin:
            for line in fin:
                ...
                image_file = splits[0]
                labels = splits[1:]
                if test_location == False:
                    self.labels.append([labels[0]])
                elif  ('E' in labels[1] or 'W' in labels[1]) and ('N' in labels[2] or 'S' in labels[2]):
                    labels[0] = (int(labels[0]))
                    lon_value = np.float64(labels[1][:-1])
                    lat_value = np.float64(labels[2][:-1])
                    if labels[1].endswith('W'):
                        labels[1] = -lon_value
                    else:
                        labels[1] = lon_value
                    if labels[2].endswith('S'):
                        labels[2] = -lat_value
                    else:
                        labels[2] = lat_value
