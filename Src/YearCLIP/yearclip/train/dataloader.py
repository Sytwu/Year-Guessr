...
class YearDataLoader(Dataset):
    """
    DataLoader for image-year datasets.
    
    The expected CSV file with the dataset information should have columns:
    - 'Picture' for the image filename,
    - 'Year' for building year.
    
    Attributes:
        dataset_file (str): CSV file path containing image names and building year.
    ...
    def __init__(self, dataset_file, dataset_folder, transform=None):
        ...
        self.images, self.years = self.load_dataset(dataset_file)
    ...
    def load_dataset(self, dataset_file):
        ...
        for _, row in tqdm(dataset_info.iterrows(), desc="Loading image paths and building years"):
            filename = os.path.join(self.dataset_folder, row['Picture'])
            if exists(filename):
