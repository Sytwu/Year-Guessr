...
class YearCLIP(nn.Module):
    def __init__(self, from_pretrained=True, queue_size=512):
        super().__init__()
        self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))
        self.image_encoder = ImageEncoder()
        self.year_encoder = YearEncoder(from_pretrained=False)

        self.year_gallery = torch.tensor([[year] for year in range(1000, 2024+1)]).float()
    ...
    def _load_weights(self):
        ...
        self.year_encoder.load_state_dict(torch.load(f"{self.weights_folder}/year_encoder_weights.pth", weights_only=True))
    ...
    def forward(self, image, year):
        """ YearCLIP's forward pass

        Args:
            image (torch.Tensor): Image tensor of shape (n, 3, 224, 224)
            year (torch.Tensor): Building year tensor of shape (m, 2)
        ...
        year_features = F.normalize(year_features, dim=1)
        
        # Cosine similarity (Image Features & Year Features)
        logits_per_image = logit_scale * (image_features @ year_features.t())

        return logits_per_image

    @torch.no_grad()
    def predict(self, image_path, top_k):
        """ Given an image, predict the top k building years
        ...
        Returns:
            top_pred_year (torch.Tensor): Top k building year of shape (k, 1)
            top_pred_prob (torch.Tensor): Top k year probabilities of shape (k,)
        """
        image = Image.open(image_path)
        image = self.image_encoder.preprocess_image(image)
        image = image.to(self.device)

        year_gallery = self.year_gallery.to(self.device)

        logits_per_image = self.forward(image, year_gallery)
        probs_per_image = logits_per_image.softmax(dim=-1).cpu()

        # Get top k predictions
        top_pred = torch.topk(probs_per_image, top_k, dim=1)
        top_pred_year = self.year_gallery[top_pred.indices[0]]
        top_pred_prob = top_pred.values[0]

        return top_pred_year, top_pred_prob
